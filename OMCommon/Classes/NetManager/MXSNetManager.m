...
progressBlock:(MXSDownloadProgressBlock)progressBlock
{
    ...
    {
        sessionTask = [MXSNetManagerShare.sessionManager GET:URLString parameters:parameters headers:nil progress:^(NSProgress * _Nonnull downloadProgress) {
    ...
    {
        sessionTask = [MXSNetManagerShare.sessionManager POST:URLString parameters:parameters headers:nil progress:^(NSProgress * _Nonnull uploadProgress) {
            if (MXSNetManagerShare.isOpenLog)
            {
    ...
    {
        sessionTask = [MXSNetManagerShare.sessionManager PUT:URLString parameters:parameters headers:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
    ...
    {
        sessionTask = [MXSNetManagerShare.sessionManager DELETE:URLString parameters:parameters headers:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            if (successBlock)
            {
                successBlock(responseObject);
            }
...
progressBlock:(MXSUploadProgressBlock)progressBlock
{
    ...
    sessionTask = [MXSNetManagerShare.sessionManager POST:URLString parameters:imageEntity.parameters headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        /*! 出于性能考虑,将上传图片进行压缩 */
...
progressBlock:(MXSUploadProgressBlock)progressBlock
{
    ...
    [avAssetExport exportAsynchronouslyWithCompletionHandler:^{
        switch ([avAssetExport status]) {
            ...
            {
                [MXSNetManagerShare.sessionManager POST:fileEntity.urlString parameters:fileEntity.parameters  headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
progressBlock:(MXSUploadProgressBlock)progressBlock
{
    ...
    MXSURLSessionTask *sessionTask = nil;
    sessionTask = [MXSNetManagerShare.sessionManager POST:fileEntity.urlString parameters:fileEntity.parameters  headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
+ (UIImage *)imageWithImage:(UIImage *)image scaledToSize:(CGSize)newSize
{
    ...
    UIImage *newImage = [UIImage rh_needCenterImage:image size:newSize scale:1.0];
